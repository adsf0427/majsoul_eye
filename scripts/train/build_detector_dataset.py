...
Writes into ``--out`` (default ``datasets/detector``) WITHOUT copying images:
    train.txt / val.txt   newline lists of ABSOLUTE image paths (native OS sep, so
                          Ultralytics recovers ``.../labels/<seq>.txt`` by swapping
                          the ``images`` path segment — this is why we do NOT
                          POSIX-ify the paths on Windows)
    data.yaml             path/train/val + the frozen 38 class names (tiles.TILE_NAMES)
...
def split_images(sources: dict, val_name: str, val_set, kyoku_fn=seq_to_kyoku):
    """sources: name -> (yolodir, capture). Returns (train, val) lists of ABSOLUTE
    native image paths. ``val_set == "*"`` holds the whole game out; otherwise a
    kyoku set from ``kyoku_fn(capture)``. Non-val games go entirely to train."""
    train, val = [], []
    for name, (yolodir, capture) in sources.items():
        ...
        for p in sorted(glob.glob(os.path.join(yolodir, "images", "*.png"))):
            ap = os.path.abspath(p)               # native sep — Ultralytics needs it
            if not want_val:
                train.append(ap)
            elif val_set == "*":
                val.append(ap)
...
def main() -> None:
    ...
    sources = {}
    for d in args.data:
        name, yolodir, capture = parse_data_arg(d)
