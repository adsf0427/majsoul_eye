...
slashes work on Windows too.
    ...
    data.yaml             relative path/train/val + the frozen 38 class names (tiles.TILE_NAMES)
...
def build_data_yaml_text(out_dir: str, train_rel: str = "train.txt",
    ...
    """data.yaml text with class names sourced from the frozen tiles.TILE_NAMES."""
    lines = [
    ...
    lines += [f"  {i}: '{name}'" for i, name in enumerate(TILE_NAMES)]
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(f"train imgs={len(train)}  val imgs={len(val)}  classes={len(TILE_NAMES)}  -> {args.out}")
    if not val:
