...
def test_data_yaml_names_match_frozen_taxonomy():
    ...
    # hud.DET_NAMES (38 tiles + 17 HUD elements + 1 reach_stick, Task 17a/17c --
    # reach stick revised to a single symmetric class, spec §10; + btn_babei
    # appended for sanma, STATUS §1.61 -> 57); v1 (pre-HUD) label files only
    # ever used ids 0-37, so the tile prefix/order must stay frozen.
    txt = bdd.build_data_yaml_text("datasets/detector")
    assert f"nc: {len(DET_NAMES)}" in txt
    names = _parse_names(txt)
    assert len(names) == len(DET_NAMES) == 57
...
def test_parse_result_hud_classes_and_out_of_range_ids():
    """57-class detector head regression (majsoul_eye.hud.DET_NAMES = 38 tiles +
    19 HUD classes, ids 38-56): a HUD-class detection must NOT raise IndexError
