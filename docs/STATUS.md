...
> 最后更新对应进度：dealfix 重训分类器 **val 0.9991**（§1.16）；**OBB 定向检测器全链打通**（§1.17，mAP50-95 0.9804 rotated-IoU）。
> （历史里程碑：16 局精确分类器 99.78% §1.14、P5 YOLO 检测器 mAP50 0.993 §1.15。）
...
§1.17：`test_detector` 扩充 OBB——`box_quad`/`obb_label_line`/`hbb_label_line` 标签几何 + `_parse_result` 的 OBB/HBB 双分支解析。）
