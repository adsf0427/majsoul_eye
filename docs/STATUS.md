...
> 最后更新对应进度：**检测器权重版本化落盘 `weights/detector/tile_detector_<mode>_<name>.pt` + OBB 自动提权为现役默认 `recognize/tile_detector.pt`（`--also-out`）**（§1.39，2026-07-06）；
> 此前：分类器纳入启动器框架 `launch_classifier.sh`（自动读 games.json val、与检测器同批留出）+ 现役切 `datasets/v2` + 清理过时一次性脚本（§1.38，2026-07-06）；
> `build_datasets.py --hbb --obb` 一次出双格式（§1.37）；`--val` 可重复留出多整局作 val（§1.36，2026-07-06）；
> `launch_detector.sh` 切版本化 `datasets/<name>` 布局 + build 收尾输出更新（§1.35，2026-07-06）；
> run_5 信箱局就地去黑边 + 撤掉 `FRAMES_OVERRIDE` 特判（§1.34，2026-07-05）；换肤局 back 门去皮肤化（§1.33）；检测器增强显式化（fliplr 关/hsv_v 提升）+ 宝牌闪光覆盖统计（§1.30，2026-07-05）；
