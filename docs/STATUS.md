...
- **Task 8 全量回归实测**（51 tests + oracle 全量 + assemble×2 + samples/，详见
  ...
  - `samples/` 16 张真实手机截图（含宽屏 >16:9）：**16/16 全过**，宽屏帧 HUD 留 null 未引入
    误拒。
- **已知缺口（留给后续，不在本轮范围）**：
  1. ~~宽屏手机帧（`region.ox>0`）HUD 字段留 null~~——**同日放开**（见本条目末尾"宽屏门控放开"）。
  ...
  3. 按钮帧 recall 缺口沿袭 §1.53（采集时序：帧先于按钮渲染，需 multishot/`--op-delay` 线补齐，
     本轮未动）。宽屏实测补充：暗皮肤 iPhone 帧（IMG_1964）吃/跳过横幅双漏检，皮肤加重此缺口。
  4. 終審 follow-up：`check_observed` 增加閉門立直守衛（reach 座持非暗槓副露 → violation，幻影
     立直棒不再產生非法 mjai；oracle 10121/10121 復跑零觸發）。
- **宽屏门控放开（同日 follow-up）**：先做检测器级探针（16 张 samples/ 直通 56 类检测器 +
  `assemble_hud`，无门控）——9 个核心 HUD 字段 **16/16 全检出**（中位置信度 0.96+，无重复框），
  抽 3 张目验读数全部与画面一致，且全 16 张分数守恒成立（含 1.png 的
  44500+18500+15200+21800=100000）、两组连拍余牌数各自严格单调递减。结论：中央面板/左上计数器
  在手机布局同构渲染（贴 3D 桌面中心），"检测器没见过手机布局"的担忧仅对宝牌（已有 stray 救援）
  和按钮成立。于是移除 `assemble` 的 `region.ox==0` 门（`test_assemble` 增宽屏填充用例），
  全链复跑 16 张：**HUD 字段 16/16 填满、15/16 ok**。唯一"回归" IMG_1964 实为交叉校验立功：
  该帧左家刚打出的 3s 带绿色高亮描边，光晕撑歪 OBB → 长边方向误判横放 → 幻影立直——旧链路
  16/16 里这帧的 mjai 静默带着不存在的 reach 事件，如今 `kyotaku 0 < 可见立直数 1` 将其显式
  拒收（宁拒不错）。**新已知问题（牌面侧，与 HUD 无关）**：最新打牌的高亮光晕可致 OBB 横放
  误判（16:9 域内 no-hud 基准的 zone_errors.reach=3 疑似同因），后续可在 sideways 判别前
  收缩光晕或参考 f1deab5 的思路加固。宽屏立直棒/座位归属仍零样本未验证。
...
### 1.55 按钮欠召回根因：亮度阈把 46% 可见按钮标成背景负样本（2026-07-10）

- **触发**：§1.54 末尾记的"宽屏实测：暗皮肤 iPhone 帧（IMG_1964）吃/跳过横幅双漏检"，
  外加一个假设——"按钮帧少，类别不均衡"。**两条都不对**：漏检是真的，但既不是类不均衡，
  也不是采集时序（§1.53 的归因）。
- **复现**：生产 OBB 权重跑 `samples/IMG_1964.PNG`，置信度地板压到 0.002：
  `btn_skip` **0.0093**、`btn_chi` **无任何激活**（运行阈 0.25）。
- **排除类不均衡**：v5 标签里失败的两类恰是最多的两类（`btn_skip` 1121、`btn_chi` 568 实例）；
  真正枯竭的是 `btn_kyushu`（**0**）、`btn_tsumo`(25)、`btn_kan`(39)。纯类不均衡的话失败顺序应反过来。
  语言也无辜：标注召回 ja 49.1% / zh-Hans 48.9% / zh-Hant 48.4%。
- **消融锁定背景上下文**（生产权重，conf 地板 0.002）：把手机暗皮肤的「吃」横幅贴到 PC 亮桌布上
  → `btn_chi` **0.922**；把 PC 自己的「チー」横幅贴到手机暗屋顶背景上 → **0.054**。
  gamma 提亮无激活；imgsz 1280→2048 只把 skip 抬到 0.199。⇒ 字形/语言/皮肤/亮度/尺度全清白。
- **根因（标注侧）**：`annotate/hud.py locate_button_candidates` 用 `gray>=140` 找亮斑，而横幅底色
  灰度只有 40–95（代码注释自己写着），抓到的其实是**字**。于是两种崩法——亮/花背景掩膜泛滥粘连、
  超 `BTN_MAX_W` 被拒（0 候选）；暗字形皮肤字不过阈。`count_mismatch` 后 `build_dataset.py:141`
  只跳过 box、**图像照留** ⇒ 画面上清清楚楚的按钮成了 YOLO 背景负样本。与 §1.33 皮肤牌背同病。
- **全量实测**（v5 全 144 局，3364 个 "GT 有按钮且有帧" 的帧）：
  - 标注侧召回 **53.9%**（默认皮肤 82.9% / 皮肤局 48.9%；BTN_ZONE 平均灰度 ≥100 的桶仅 14.8%）。
  - 与闸门无关的存在性检验（逐局 BTN_ZONE 背景中值模型，差分覆盖率）：被丢的 1551 帧里
    **≥92.8% 按钮清晰渲染**，仅 ≤7.2% 是"帧先于按钮渲染"的采集时序——**§1.53/§1.54 的归因基本是错的**。
    （阈值取"已标注帧覆盖率的 p5"，按构造会把约 5% 已渲染帧误判为未渲染，故 92.8% 是**下界**、
    7.2% 是**上界**：事后目验 `ai_session5_run_1_game41` 中覆盖率 0.1174 的 6 帧，按钮全部清晰渲染。）
  - 检测器数据集 train 分割：**1102 帧带按钮标签 vs 1009 帧有 GT 按钮却无标签**。
  - 检测器行为严格跟着标签走。**保留集 val**（无记忆效应）：闸门接受的按钮 recall **99.3%**
    （中位 conf 0.964）；闸门丢弃但已渲染的按钮 recall **0/92 = 0.0%**（中位 conf 0.012）。
    IMG_1964 的 0.0093 正落在后一个分布里——它不是特例。收紧到"渲染强度高于已标注帧中位数"的
    1394 帧仍只有 5.7% recall ⇒ 不是淡入/半透明造成的。
- **附带发现**：被**接受**的 53.9% 里，约 8% 的框本身是错的（闸门抓到立绘高光/风景亮斑，
  目验 `btn_chi` 框飘到两块牌子中间）；且 `banner_box` 虽然输出恒定 250×96，却**以字形质心为锚**，
  于是 §1.47 想摆脱的语言依赖以位移形式残留：skip 框中心 ja 0.6812 vs zh-Hans 0.6755（差 11px），
  且整体比真牌子中心右偏 16px。

**修复（本轮，两条独立可验证）**

1. **数据卫生**（`build_dataset.has_unlabeled_buttons` + `tests/test_button_hygiene.py`）：
   任一 `hud_boxes` 带 `flag:count_mismatch` ⇒ 该帧**整帧不进检测器 YOLO 集**（图像与标签都不写；
   分类器裁剪不受影响，牌面标签本身没问题）。永久安全网：可见目标绝不再以背景身份进入训练。
   `build_dataset` 统计行新增 `btn-mismatch-dropped`。
2. **牌子分割取代亮度阈**（`annotate/btnbg.py` + `hud.locate_button_plates`/`plate_banner_box`；
   `tests/test_btnbg.py`、`tests/test_button_plates.py`）：按钮是画在静态桌面上的 overlay ⇒
   `|frame − 本局 BTN_ZONE 背景中值|` 直接分割**牌子本体**，无亮度阈、无语言/皮肤假设。
   背景模型 = 该局 GT 无按钮帧的逐像素中值（均匀采样 ≤31 帧，中值抹掉动态桌布/雨雪/立绘呼吸）。
   `annotate_frame(..., btn_bg=)` 透传，`annotate_ai_session.py` / `build_dataset.py` 两个管线入口
   各自逐局构建；无背景模型时退化回旧亮度阈（仅 overlay/inspect 工具会走到）并打 WARN。
   框心取牌子质心而非字形质心 ⇒ 语言偏移消失。

**标定**（30 局，混合皮肤 + ja/zh-Hans/zh-Hant，均实测非拍脑袋）：牌子连通域 w 151–248（中位 205）、
h 66–105（中位 89）、area 6379–18373；**相邻牌子最小间距 39px** ⇒ 闭核宽取 21 不会桥接两个按钮。
阈值刻意放宽于实测范围（`PLATE_MIN_W=120` 等），因为边缘融合会缩小连通域——卡太紧就是重造同一个 bug。

**验收**（真实代码路径，30 局）：标注侧按钮召回 **63.9% → 94.9%**（该抽样的旧值；全量旧值 53.9%），
`count_mismatch` 整帧丢弃率 **35.7% → 4.7%**（真未渲染帧的实际占比在此量级）；无按钮帧误报 **0.06%**；
牌子质心跨语言一致（ja 0.6784 / zh-Hans 0.6784 / zh-Hant 0.6786）。单测 55/55 全绿。

**⚠️ 数据 STALE**：`datasets/v1`–`v5` 的按钮标签全部过期（且 v1–v5 的检测器集含那批背景负样本）。
需 `annotate_ai_session` 重跑 + `build_datasets.py <v> --force` + 重训检测器，之后才谈得上在
`samples/IMG_1964` 与 val 的那 92 个按钮上复验。**本轮不含重训**（GPU 长作业，另行触发）。

**遗留（不在本轮）**：`btn_kyushu` 训练集 0 例、`btn_tsumo` 25、`btn_kan` 39 —— 这才是真正的类
...
### 1.57 重建链跟进新 OBB 权重：drawn 竞态 join 修复 + 立直宣言窗口支持（IMG_1964 误诊纠正）（2026-07-10）

新权重 `tile_detector_obb_20260710_132055`（§1.55/§1.56 标注修复后的重训）首基准：run_8 game1
带 HUD **912/928 全等**（旧权重同代码 907/931），`reach`/`rivers`/`melds` zone error 全部清零，
唯一残余 `drawn: 16`。逐帧取证后本轮落了两个重建侧修复：

- **drawn 的 16 帧全是评测 join 竞态，不是视觉链错误**：帧在英雄摸牌动画渲染完成后才满足
  quiet 采集，却被 key 到摸牌**前**一条记录（与 §1.53 wall_count 像素=GT−1 同一采集时序病根）。
  像素侧 14 张含真实分离摸牌槽（末位间隔 0.064 > 阈 0.057），手牌/河与 GT 逐张全等，目验
  001059.png 确认。修复（eval 侧）：`run_assemble` 后继状态**精确匹配**才重 join —— 条件
  `gt.drawn is None` + `obs.drawn` 非空 + 后继投影 drawn 相同且 zone 全等；计数器 `drawn_race`。
  ⚠️ **数据集侧同病未修**：这些帧在 v1–v5 里 GT=13 张 → 摸牌槽上那张真实渲染的牌无标签 →
  检测器背景负样本（~1.7% 帧，§1.55 病族"把画面上的东西标成背景"）。annotate/build 无法单靠
  GT 识别此竞态（需像素探针或采集侧修正），留待下轮重标注一并处理。
- **IMG_1964 的"幻影立直"是误诊——§1.54 的记载有误**：fullwarp 目验，那张 3s 的图案方向与
  同河邻牌相差 90°（seat3 直立牌在俯视图里字应横躺，它却是竖的）——**它真是横放的立直宣言牌**；
  截图恰拍在宣言动画内（立直棒未放、千点未扣、kyotaku 未加，四家都还 25000）。检测器与 OBB
  长边判别全对；被 `kyotaku < 可见立直数` 守卫拒掉的其实是一个合法的真实决策点（他家荣/鸣
  尚未决）。顺带实测（为排除几何守卫方案收的数据）：34k 张直立河牌 OBB 边比 min 1.226、
  带光晕的真横牌低至 1.135，但长边方向判决在全部 31 个低边比样本上仍全对——**不需要几何守卫**。
- **立直宣言窗口支持（本轮核心）**：`check_observed` 放行"缺口恰好 1 且该座横牌是其河最新
  一张"（其余缺口照拒：≥2、横牌非最新、或宣言后有人行动）；`reconstruct._search` 新增
  `pending_reach` 约束——宣言打牌必须是序列**最终事件**（不可 ghost 绑定、与 hero_call_pending
  互斥、hero 不得持摸牌）；`_emit` 对该座发 `reach`+`dahai` 但**不发 `reach_accepted`**，回填
  不计其 1000 棒（start_kyoku 分数/kyotaku 保持所见值）；诊断 `pending_reach_seat`。
- **验收**：域内 **932/932 接受帧 100% 全等**（949 帧、98.2% 接受；拒收 21→17，`hud_kyotaku`
  类目清零，score-anim 拒收 12→8——余 8 为 HUD 中间渲染态，像素真不一致，拒收正确）；oracle
  全量 **10121/10121** 零漂移（209 call-pending 照旧）；手机样本 **15/15**（IMG_1964 复活，
