...
- **分类器已在清洗后数据上重训并晋升生产（2026-07-03）**：`tile_classifier.pt` = 发牌修复权重，held-out **best val_acc=0.9991**（原 `_dealfix` 快照，现已晋升覆盖生产文件）。检测器 `tile_detector.pt` **暂未在清洗后数据上重训**（仍为 §1.15 旧权重；重训命令见下）。
- **测试**：`test_replay` 加 `is_deal_window`（打包后 `last_event=='tsumo'` 仍判 True→首弃翻 False）；`test_sync` 加 note 发牌窗抑制。11 套仍全绿。

### 1.17 权重仓瘦身：只保留生产分类器 + 历史清除备份（2026-07-03）
