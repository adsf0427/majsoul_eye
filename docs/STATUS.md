...
> 最后更新对应进度：**分类器纳入启动器框架 `launch_classifier.sh`（自动读 games.json val、与检测器同批留出）+ 现役切 `datasets/v2` + 清理过时一次性脚本**（§1.38，2026-07-06）；
> 此前：`build_datasets.py --hbb --obb` 一次出双格式（§1.37）；`--val` 可重复留出多整局作 val（§1.36，2026-07-06）；
