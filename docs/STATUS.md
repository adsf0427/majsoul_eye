...
> 最后更新对应进度：**HUD 检测（分数/场风/局/余牌/供托/本场/动作按钮）代码落地——55 类
> detector taxonomy + 微读取器 + 按钮/字段标定 + 采集侧 multi-shot/`--op-delay`/
> `is_call_window`，代码全通、训练待跨分支数据集合并**（§1.31，2026-07-06）；
> 此前：采集截图黑边 bug 修复（CDP 截图 clip 到视口）+ run_5 离线裁回 1920×1080（§1.30，2026-07-05）；
> 服务器侧 `regen_detector_dataset.sh` 切嵌套布局 + 训练启动器文档（§1.29）；skins 元数据 hero 修正 + ai_session2 回填（§1.28）；采集统一 AI 路线 + 数据集版本化（§1.19–§1.22，2026-07-04）。
...
### 1.30 采集截图黑边 bug：CDP 截图 clip 到视口 + run_5 离线裁回（2026-07-05）
...
### 1.31 HUD 检测：55 类 taxonomy + 微读取器 + 按钮/字段标定 + 采集侧扩展（代码全通，训练待排）（2026-07-06）
...
`_shot_clip`（CDP 截图 clip，见 §1.30）WIP 一并原样提交。
...
## 三、数据集现状（2026-07-04 全量重建 + run_13/14 补建）

| 层 | 内容 |
|---|---|
| 原始 | `captures/raw/ai_session/` **18 AI 局**（run_1；run_3×4；run_4×1 掉线局；run_5×3，其中 2 局信箱 → `derived/*_fixed` 修复帧；run_7×1；run_8×6；run_13/14 早退迷你局各 15 帧）+ `raw/manual/` session5/6（4K 手动，**采集方式已过时**、数据保留） |
| 数据集 | **`datasets/v1/`（版本化，§1.22）**：每局子文件夹 `precise_<game>` ×20（crops + yolo，~38w+ crops）+ `detector/` **train 8683 / val 949**（held-out 整局 `ai_run_8_game1`）+ `annotations/` 标注缓存 + `games.json` 清单（manual 局直接建库，不经标注层） |
| 权重 | 正式 `recognize/tile_classifier.pt`（val 0.9991，07-03 数据）+ `recognize/tile_detector.pt`（HBB 0.993/0.955）；变体 `weights/detector/tile_detector_obb.pt`（0.9804）等 |

> ⚠️ 两模型尚未在 07-04 重建数据上重训（命令见 §五 0c；`build_datasets.py` 收尾也会打印）。
...
- **HUD 区**（分数/场风/局/余牌/供托/本场/动作按钮）：设计稿
  ...
  **代码已落地**（§1.31，2026-07-06：55 类 taxonomy + 字段/按钮标定 + 采集侧 multi-shot/
  `--op-delay`/`is_call_window`）；**训练未跑**——`train_hudreader.py`/55 类 `train_detector.py`/
  真实 `qa_hud.py` 都待用户另一分支完成数据集合并（v1+v3 混训）后再执行。玩家名 OCR 仍出范围。
